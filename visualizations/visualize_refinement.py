#!/usr/bin/env python3
"""
Visualize model refinement for ALL agents in a scene simultaneously.

All agents in the scene are batched through the same refinement loop, and every
frame of the GIF shows their predictions in a shared world-coordinate BEV frame.

Usage:
    python visualizations/visualize_refinement.py \
        --config  configs/basic_config.yml \
        --checkpoint checkpoints/basic.pth \
        --output  basic_scene_refinement.gif \
        --min_agents 4
"""
...
try:
    ...
    from AnytimeTrajectoryPredictor.models.architectures.astra_edm_refinement import make_karras_sigmas
...
# ── refinement sampling with per-step world-space capture ─────────────────────
...
def render_frame(frame: dict, step_idx: int, total_steps: int, is_final: bool,
    ...
    """Render all agents in world coordinates for one refinement step."""
...
def _render_rgb_prediction_frame(frame: dict, ctx: dict, step_idx: int, total_steps: int) -> Image.Image:
    ...
    if step_idx == 0:
        ...
        title = f"refinement {step_idx}/{total_steps}  sigma={frame['sigma']:.4f}"
...
def make_rgb_overlay(
    ...
    print("Running refinement sampling …")
...
def make_gmm_png(
    ...
    """Produce a single static GMM visualization (no refinement animation)."""
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")
    ...
    # Run full refinement — only the last frame is used
    frames   = sample_scene(model, agents, device, num_steps=num_steps)
    final_fr = frames[-1]
...
def make_gif(
    config_path: str,
    ckpt_path: str,
    output_path: str = "scene_refinement.gif",
    scene_id: str | None = None,
    min_agents: int = 3,
    max_agents: int = 20,
    max_segs: int = 5,
...
def main():
    p = argparse.ArgumentParser(
        description="Visualize model refinements for all agents in a scene.\n"
                    "  --mode gif   → animated refinement GIF (default)\n"
    ...
    p.add_argument("--output",       default="scene_refinement.gif",
